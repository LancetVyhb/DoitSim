...
public class DeleteJoinByCloud {
    ...
    public void convertData() {
        ...
        for (ProjectItems items : itemsList)
            if (items.getType() != 2 && items.getChildren() != null)
                itemsList.addAll(items.getChildren());
    ...
    private void setCloudAsc(){
        deleteJoin = new CloudCodeListener() {
            @Override
        ...
        upload.setThread(new Runnable() {
            @Override
            public void run() {
                asc.callEndpoint("deleteJoin",params,deleteJoin);
            }
