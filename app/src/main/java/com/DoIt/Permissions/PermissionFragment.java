...
public class PermissionFragment extends Fragment {
    ...
    public void requestPermissions(@NonNull String[] permissions) {
        ...
        if (requestPermissionList.isEmpty()) {
            ...
            //申请授权
            requestPermissions(requestPermissionList.toArray(new String[0]), PERMISSIONS_REQUEST_CODE);
        }
    }
    /**
     * fragment回调处理权限的结果
    ...
    private void permissionHasDenied(List<String> deniedList) {
        if (listener != null) listener.onDenied(deniedList);
    }
    /**
     * 权限被拒绝并且勾选了不在询问
    ...
    private void permissionShouldShowRationale(List<String> deniedList) {
        if (listener != null) listener.onShouldShowRationale(deniedList);
    }
}